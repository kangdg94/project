...
@Aspect //AOP 클래스 선언: 부가 기능으르 주입하는 클래스
@Component //IOC 컨테이너가 해당 객체를 생성 및 관리
@Slf4j
public class DebuggingAspect {

    //대상 메소드 선택: CommentService#create()
    @Pointcut("execution(* com.example.kangdg.api.*.*(..))")
    private void cut(){
    }
    ...
    @AfterReturning(value = "cut()",returning = "returnObg")
    public void loggingReturnValue(JoinPoint joinPoint, //cut()의 대상 메소드
        ...
        log.info("{}#{}의 반환값 => {}", className, methodName, returnObg);
